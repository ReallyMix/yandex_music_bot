...
class TestMusicServiceAddTracks:
    ...
    @pytest.mark.asyncio
    async def test_add_tracks_by_name_success_existing_playlist(self, music_service):
        """Тест успешного добавления треков в существующий плейлист"""
        token = "test_token"
        user_id = 123
        ...
        with patch.object(music_service, "get_client", return_value=mock_client):
            with patch.object(music_service, "_get_account_uid", return_value=123456):
                with patch.object(
                    music_service, "_find_playlist_by_title", return_value=mock_playlist
                ):
                    with patch.object(
                        music_service,
                        "_search_track_id",
                        side_effect=["123:456", "789:101"],
                    ):
                        ...
                        expected = {
                            "added": [
                                {"query": "Song 1", "track_id": "123:456"},
                                {"query": "Song 2", "track_id": "789:101"},
                            ],
                            "failed": [],
                        }
